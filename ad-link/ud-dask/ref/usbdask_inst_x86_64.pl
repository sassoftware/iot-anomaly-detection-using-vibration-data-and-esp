...
if( $ARGV[0] eq "" )
{
    $CONF_FILE_NAME = "../drivers/usbdask.conf";
}
else
{
    if( $ARGV[0] =~ /\/$/){
...
if( $DRIVER_PATH ne "" )
{
    $INSMOD_COMMAND ="cp -f " . $DRIVER_PATH . "/drivers/firmware/*.bin /etc/udask/fw";
}
else
{
    $INSMOD_COMMAND ="cp -f ../drivers/firmware/*.bin /etc/udask/fw";
}
`$INSMOD_COMMAND`;
#======================================================================
...
if( $DRIVER_PATH ne "" )
{
    ...
    if( -d "/usr/lib64" ){
        $INSMOD_COMMAND ="cp -f " . $DRIVER_PATH . "/libsrc/usb_dask64/libusb_dask64.so /usr/lib";
    }
    elsif( -d "/usr/lib/x86_64-linux-gnu" ){
        $INSMOD_COMMAND ="cp -f " . $DRIVER_PATH . "/libsrc/usb_dask64/libusb_dask64.so /usr/lib/x86_64-linux-gnu";
    }
...
else
{
    ...
    if( -d "/usr/lib64" ){
        $INSMOD_COMMAND ="cp -f ../libsrc/usb_dask64/libusb_dask64.so /usr/lib";
    }
    elsif( -d "/usr/lib/x86_64-linux-gnu" ){
        $INSMOD_COMMAND ="cp -f ../libsrc/usb_dask64/libusb_dask64.so /usr/lib/x86_64-linux-gnu";
    }
...
# copy the libthermo64.so to /usr/lib64
if( $DRIVER_PATH ne "" )
{

    if( -d "/usr/lib64" ){
        $INSMOD_COMMAND ="cp -f " . $DRIVER_PATH . "/libsrc/thermo64/libthermo64.so /usr/lib";
    }
    elsif( -d "/usr/lib/x86_64-linux-gnu" ){
        $INSMOD_COMMAND ="cp -f " . $DRIVER_PATH . "/libsrc/thermo64/libthermo64.so /usr/lib/x86_64-linux-gnu";
    }
...
else
{
    ...
    if( -d "/usr/lib64" ){
        $INSMOD_COMMAND ="cp -f ../lib/libthermo64.so /usr/lib";
    }
    elsif( -d "/usr/lib/x86_64-linux-gnu" ){
        $INSMOD_COMMAND ="cp -f ../lib/libthermo64.so /usr/lib/x86_64-linux-gnu";
    }
...
if( $DRIVER_PATH ne ""){
    $INSMOD_COMMAND = "cp " . $DRIVER_PATH . "/drivers/udev-rules/60-usbdaq.rules /etc/udev/rules.d";
}
else{
    $INSMOD_COMMAND = "cp /udev-rules/60-usbdaq.rules /etc/udev/rules.d";
}
`$INSMOD_COMMAND`;
...
if( $MEM_MGR_LOAD == 1 ){
    if( $DRIVER_PATH ne "" ){
        $INSMOD_COMMAND = "insmod " . $DRIVER_PATH . "adl_mem_mgr.o";
    }
    else{
        $INSMOD_COMMAND = "insmod adl_mem_mgr.o";
    }
    `$INSMOD_COMMAND`;
}
...
foreach $cardtype (keys(%DRV_NAME))
{
    ...
    if( $BUFSIZE_NEED == 1 ){
        print(" insmod $drv_name \n");
        if( $DRIVER_PATH ne "" ){
            $INSMOD_COMMAND ="insmod " . $DRIVER_PATH . $drv_name . " BufSize=$AI_PAGE{$cardtype},$AO_PAGE{$cardtype},$DI_PAGE{$cardtype},$DO_PAGE{$cardtype}";
        }
        else{
            $INSMOD_COMMAND = "insmod " . $drv_name . " BufSize=$AI_PAGE{$cardtype},$AO_PAGE{$cardtype},$DI_PAGE{$cardtype},$DO_PAGE{$cardtype}";
        }
        `$INSMOD_COMMAND`;
    }else{
        print(" insmod $drv_name \n");
        if( $DRIVER_PATH ne "" ){
            $INSMOD_COMMAND ="insmod " . $DRIVER_PATH . $drv_name;
        }
        else{
            $INSMOD_COMMAND = "insmod " . $drv_name;
        }
        `$INSMOD_COMMAND`;
    }
}
